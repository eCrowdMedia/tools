...
var m = function()
{
    ...
    var isAbsolute = function(p) { return path.normalize(p) === path.normalize(path.resolve(p)); }
    ...
    this.build = function(config, callback, log_output)
    {
        ...
        var build_path = isAbsolute(config.build_path) ? config.build_path : cwd + '/' + config.build_path.replace(/\/$/, '');
        var app_path = path.resolve('/', build_path + '/' + config.name + '.app');
        var steps = [
            function(next)
            {
                log_output ? console.log('Installing base app...') : null;
                var nwjs_path = isAbsolute(config.nwjs_path) ? config.nwjs_path : cwd + '/' + config.nwjs_path;
                Files.installBaseApp(nwjs_path, app_path, next);
            },
            function(next)
            {
                log_output ? console.log('Copying app...') : null;
                var source_path = isAbsolute(config.source_path) ? config.source_path : cwd + '/' + config.source_path;
            ...
            function(next)
            {
                ...
                var icon_path = isAbsolute(config.icon_path) ? config.icon_path : cwd + '/' + config.icon_path;
